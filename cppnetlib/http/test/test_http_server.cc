...
void HelloCallback(const HTTPRequest& request, HTTPResponse* response) {
  LOG_INFO << "Request Line: " << request.method_string()
            << " " << request.path();
  if (!benchmark) {
  ...
  if (request.path() == "/") {
    response->set_status_code(HTTPResponse::kOK);
    response->set_status_message("OK");
    response->SetContentType("text/html");
    response->AddHeader("Server", "cppnetlib");
    std::string now = cppnetlib::TimeAnchor::Now().ToStr();
    response->set_body("<html><head><title>This is title</title></head>"
    ...
    response->AddHeader("Server", "cppnetlib");
    response->set_body(std::string(favicon, sizeof(favicon)));
...
int main(int argc, char* argv[]) {
  ...
  EventLoop loop;
  HTTPServer server(&loop, listen_addr, "http");
  if (argc >1) {
    benchmark = true;
    cppnetlib::Logger::set_loglevel(cppnetlib::Logger::WARN);
    num = atoi(argv[1]);
  }
  server.set_thread_num(num);
  server.set_http_callback(HelloCallback);
